...
این لایه قابلیتی است توانایی **خواندن، نوشتن، ترجمه و فرمت‌بندی فارسی** را در هر کاری افزایش می‌دهد. نه یک معلم. نه یک شخصیت چت‌بات. یک ضرب‌کننده کیفیت برای هر گردش کاری که شامل فارسی می‌شود.

### ✨ ویژگی‌ها

- **یونیکد صحیح فارسی**: ک (نه ك)، ی (نه ي)، نیم‌فاصله (U+200C)
- **فرمت‌بندی مناسب**: ارقام فارسی (۰۱۲۳۴۵۶۷۸۹)، نشانه‌گذاری («»، «»، ؟)، پشتیبانی RTL
- **ظرافت فرهنگی**: تشخیص تعارف، تطبیق ثبت (رسمی/غیررسمی)، تاریخ‌های هجری خورشیدی
...
- **۶ فایل مرجع**: راهنماهای جامع درباره استانداردهای نوشتاری، ارقام، ثبت لحن، اشتاهای رایج و قالب‌های محتوا
...
|---------|---------|-------------|
| `validate.py` | تشخیص خطاهای یونیکد، مسائل فرمت‌بندی | **همیشه** - قبل از برگرداندن هر خروجی فارسی |
| `validate.py --semantic-hints` | اضافه کردن چک‌لیست بازبینی (کالک، ترتیب کلمات، اضافه، تعارف) | **خروجی غیرساده** - ترجمه‌ها، متن‌های بلند |
| `lookup.py` | جستجوی Wiktionary + ترجمه (کش ۳۰ روزه) | **در صورت نیاز** - وقتی در انتخاب کلمه مطمئن نیستید |
